...
# Define data structure for q-table and define the discount factor and epsilon
...
while not exit_program:
    ...
    if runai:
        ...
        # 1. choose an action
        current_state = (x, y, has_key)
        action_num = np.argmax(Q[current_state])
        action = actions[action_num]  

        # 2. step the environment
        (x, y, has_key), reward, done = env.step(action)
        next_state = (x, y, has_key)
